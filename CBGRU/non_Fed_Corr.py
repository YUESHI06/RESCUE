...
from data_processing.dataloader_manager import gen_whole_dataset, gen_cbgru_valid_dl
...
if __name__ == "__main__":
    ...
    whole_ds, input_size, time_step, data_indices = gen_whole_dataset(args.client_num, args.vul, noise_types, noise_rates)
    criterion = nn.CrossEntropyLoss(reduction='none')
    LID_accumulative_client = np.zeros(args.client_num)

    # set Server
    global_model = ClassiFilerNet(input_size, time_step)
...
# ---------------------------- third stage training -------------------------------
    ...
    if args.noise_type != "diff_noise":
        args.noise_rate = 2.3
    global_test(server.global_model, test_dl, criterion, args, "non_Fed_Corr", 'none')
