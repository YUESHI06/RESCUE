...
if __name__ == "__main__":
    ...
    for i in range(args.client_num):
        noise_labels = get_noise_labels(i, args.vul, args.noise_type, noise_rates[i])
        noise_labels_list.append(noise_labels)
        noise_dl = gen_client_dataloader(i, args.vul, args.noise_type, noise_rates[i], noise_labels)
        dataloader_dict['noise'][i] = noise_dl
        pure_dl = gen_client_pure_dl(i, args.vul, args.noise_type, noise_rates[i])
    ...
    for epoch in range(args.epoch):
        ...
        for client_id in range(args.client_num):
            ...
            noise_dl = gen_client_noise_dl(client_id, args.vul, args.noise_type, conc_labels, noise_labels_list[client_id], noise_rates[client_id])
    ...
    args.noise_rate = 2.3
    CGE_test(server.global_model, test_dl, criterion, device, args, 'non_Fed_PLE')
